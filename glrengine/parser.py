...
class RuleSet(dict):
    ...
    def init(self, rules):
        ...
        while epsilons:
            eps = epsilons.pop()
            if eps in self:
                # Правило производит что-то и имеет эпсилон альтернативу
...
class Parser(object):
    ...
    def itemsetstr(self, item, label=''):
        """
            Stringify - пункт для красивой печати.
        """
        return itemsetstr(item, self.R, label)

    def closure(self, s):
        """
            Вычислить закрытие набора элементов.
        """
        return tuple(sorted(closure(s, self.R)))

    def kernel(self, s):
        """
            Вычислить ядро набора предметов.
        """
        return kernel(s)

    def index(self, s):
        """
            Возвращает индекс (ближайший) набора элементов s в списке наборов LR (0).
        """
        return self.LR0_idx[self.closure(s)]

    def compute_GOTO(self):
        """
            Вычислить таблицу GOTO.
        """
        self.GOTO = []
        for s in self.LR0:
            f = {}
    ...
    def init_row(self, init=None):
        """
            Инициализация строки таблицы ACTION.
        """
        if init is None:
            init = []
        ret = {}
    ...
    def next_items(self, item, visited=None):
        """
            Вычислить еще не посещенные элементы, следующие за данным элементом.
        """
        items = set()
        if visited is None:
            visited = set()
    ...
    def following_tokens(self, item):
        """
            Возвращает все токены, следующие за текущим элементом.
        """
    ...
    def action_to_str(self):
        """
            Stringify - таблицу ACTION/GOTO для красивой печати.
        """
    ...
    def dump_sets(self):
        """
            Красивая печать для всех LR(0) наборов.
        """
