...
def token_line_col(text, tok):
    """
        Преобразует токен в (строка, столбец).
        Первый символ - (1, 1).
    """
    line = text.count('\n', 0, tok[2]) + 1
    offset = text.rfind('\n', 0, tok[2])
...
class GLRScanner(object):
    ...
    def add(self, **tokens):
        """
            Каждое именованное ключевое слово является типом токена,
            а его значением является соответствующее регулярное выражение.
            Возвращается функция, которая перебирает токены в форме (тип, значение) по строке.

            Специальными ключевыми словами являются discard_names и discard_values,
            которые определяют списки (на самом деле принимаются любые итерируемые),
            содержащие имена токенов или значения,
            которые должны быть отброшены из вывода сканера.
        """
        ...
        # Проверка, что в утверждении нет неопределенной группы
    ...
    def __call__(self, text):
        """
            Итеративно просматривает текст и выдает каждый токен
        """
        pos = 0
        states = [None]
        while True:
            ...
            pos = m.end()
            # определяем, что перед нами (слово? знак препинания?)
            tokname = m.lastgroup
