...
def first(itemset, R):
    """
        Набор токенов справа от каждой точки в этом наборе данных (множестве)
    """
    ret = set()
...
def follow(itemset, R):
    """
        Все переходы из набора элементов в словаре [token]->item set
    """
    ret = dict()
...
def closure(itemset, R):
    """
        Эпсилон-окрестность(дерево?) этого набора
    """
    C = set(itemset)
    last = -1
    visited = set()
...
def kernel(itemset):
    """
        Элементы ядра в этом наборе элементов
    """
